...
unsigned int find_dim_id(pugi::xml_node parent){
	
	auto dim_list = parent.children("dimensionNode");
	
	unsigned int max_id = 1;
	for(auto i = dim_list.begin(); i != dim_list.end(); ++i){
		if(i->child("dimensionId").text().as_int() > max_id) max_id = i->child("dimensionId").text().as_int();
	}
	return ++max_id;
}
	...
	if (dim_open) {
		if (ImGui::Begin("Declension dimensions", &dim_open)) {
			...
			if (ImGui::Button("New")) {
				...
				dn.append_child("dimensionId").text().set(find_dim_id(pos_curr_decl));
			}
			ImGui::SameLine();
